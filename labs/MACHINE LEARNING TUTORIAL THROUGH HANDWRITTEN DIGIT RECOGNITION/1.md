...
This learning lab can be used as a guide to get a high level understanding to use machine learning in order to letˇdata drive decisioning. We will use a dataset of tens of thousands of images of handwritten numbers, allowing the models to learn how to decide what digit each image represents.
This is aˇsupervised classificationˇmodel, since we are trying to group the images into any of the digits 0 through 9 and we have labels for those digits.
This task, like many machine learning and artificial intelligence tasks, has powerful practical implications. Here we are using theˇMNIST dataset, which wasˇcollected by the United States Census Bureau. Many organizations, such as theˇUS Post Office, are using machine learning algorithms currently in order to provide more accurate and efficient reading of handwritten addresses and zip codes.
...
Click on ‚ÄúRegister Account‚Äù button and create a new login profile. Please refer the screen below:
...
Theˇindexˇof the image is referring to which image number, of the 10,000 available, you are looking at.
...
Machine learning isˇdata-driven modeling. As such, the data is the most precious part. We need to use the dataˇbothˇto train / create our model, as well as to validate the model.
Toˇprevent overfitting, as discussed in the lecture portion of this lab, we need to validate on an entirely different data set than that we use to train.
...
Below the modeling begins. We are usingˇsupport vector machineˇ(SVM) models. We are performing aˇsupervised classificationˇmodel. The model isˇsupervisedˇbecause we have a specified goal, and training data which provide answers to that goal. Specifically, we have handwritten digits which are to be recognized as particular digits, and we have the answers to those digits, as shown in the image above.
The model isˇclassifyingˇbecause there are particular, discrete bins that we are trying to fit the predictions into. Here we are trying to tell to which of the numbers 0 - 9 the handwritten digits look most similar. This might seem like regression, since we are talking about numbers, but it is classification, because each number looks drastically different from its neighbor. The number '1' mightˇseemˇsimilar to '0' or '2' in our minds, but as it is written, they look nothing alike. In fact, '0' looks most similar to '8', and maybe '1' looks most similar to '7'. We'll see, in the validation, that in fact the most frequent mix-ups are with digits thatˇlookˇmost similar.
Find Model Parameters
We will first run several differentˇsupport vector machineˇ(SVM) models, changing the parameters to find which work best. Specifically, we look at aˇpolynomialˇkernel versus aˇradialˇkernel, as well as several different values for theˇpenalty parameter, 'C'.
When modeling, we only use the training data. The remaining data we will use for final validation for assessing the model quality at the end.
Necessarily, this work also generates the model as well.

The modeling performs a 3-foldˇcross-validation, further breaking the training dataset into 2 groups, one of which is used for model cross-validation. This is different from the final validation mentioned immediately above.
The code below takes aˇlongˇtime to run, potentially up to 10 minutes.
```json
%python
...
We will first look at theˇconfusion matrix, a fancy phrase for a fairly simple concept.

Along the columns, we have the count of images with thatˇpredictedˇdigit, and across the rows, we have the count for theˇactualˇdigit. For instance, the upper-left corner represents the number of images that were labelled as 0's, and that were successfullyˇpredictedˇto be zeros. The upper-right corner represents the number of images that were labelled as 0's and that wereˇincorrectly predictedˇto be 9's.

Therefore, theˇdiagonalˇfrom upper-left to lower-right represents all of the digits that wereˇpredicted properly, or which the machine learning algorithm properly modelled. Anythingˇnotˇon the diagonal was improperly modeled.
...
There are several other methods to validate the quality of the model. The method given below is very powerful, but as we delve further into these mathematical methods, they become more complex. We won't delve into all of it, but we will touch upon theˇprecisionˇandˇrecall.

Wikipediaˇhas a nice discussion on the precision and recall, given below. In short, theˇprecisionˇis the percent of things that you predicted that were correctly predicted. E.g., if 500 images wereˇpredictedˇto be 0's, and 490 of themˇwere actually labelled as zeros, then the precision would be 98% (490 / 500).
The recall is the percent of things that truly were a value, that you correctly predicted to be that value. E.g., if 1000 images wereˇlabelledˇas 0's, and 950 of themˇwere predicted as zeros, then the precision would be 95%.
...
2. When we looked at the performance of the model, weˇonlyˇcompared the predictive capabilities of the validation dataset, the dataset that was never used for any modeling in any way.
Let's break that second rule above, and instead evaluate the performance by looking at how well the model works in predicting theˇtraining dataset. This will lead us to overconfidence of the quality of our models, when evaluating, and when internally assessing, it could also allow for overfitting.
...
Overall the precision and recall values areˇmuchˇcloser to 100%. But this does not represent the truth. We will never want to know how well a model can predict data we have already fed it, but instead future data that is being seen for the first time by the model.
Delving into Misclassified Images
Finally, let's look at some of those images that were misclassified (within the test set).
It's often worthwhile to look at the images that were misclassified, to better understand what might have gone wrong. Were the misclassifications because some of the handwritten digits were justˇreallyˇhard to read? Were they misclassified because there is some systematic mistake happening, such as 2's and 3's consistently being mistaken? Sometimes it helps simply to drive down into some of the errors.
...
In this lesson we walked throughˇmachine learning modeling, where weˇpredicted handwritten digits, automatically assigning a value to the handwritten digit. This sort of task is not only useful, but already being used within the industry, such asˇby the post office to better deliver mailˇto its customers.
This work involved:
1. Some preparatory work that we did not delve deeply into, such as reading the data.
o The code is available by expanding the hidden paragraph near the top.
...
o This was done either with a wider search for model parameters, or using preset parameters.
5. Predicting digits based upon theˇtest datasetˇimages.
6. Examining the quality of those predictions in multiple ways.
o Including a slight divergence to show an inaccurate means of assessing quality by using the training dataset.
